...
const getNumberFormatKind = (numFmt?: string) => NUM_FMT_TO_KIND[numFmt ?? "@"] ?? "text"
...
export function CellFormattingContextMenu() {
  ...
  const themeColorClassName = resolvedTheme === "dark" ? "dark text-foreground" : "text-foreground"
  ...
  const [fontSizeDraft, setFontSizeDraft] = useState(String(selectedStyle.fontSize || 11))
  ...
  return (
    <>
      <ContextMenuSeparator />
      <ContextMenuSub>
        <ContextMenuSubTrigger>Formatting</ContextMenuSubTrigger>
              ...
              <div className="flex gap-1">
                {ALIGNMENT_OPTIONS.map((option) => (
                  <Button
                    ...
                    type="button"
                    size="xs"
                    variant={selectedStyle.hAlign === option.value ? "default" : "outline"}
                    className="flex-1"
                    onClick={() =>
